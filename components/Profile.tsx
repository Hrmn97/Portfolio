...
function Hero({pageInfo}: Props) {
  const [text, count] = useTypewriter({
    words: [`Hi, from Web Developer ${pageInfo?.name}`,"Tech geek love to build","Fresher available to learn new tech","<Code />"], loop:true, delaySpeed:2000
  })
  return (<>
    
    <div className='h-screen flex flex-col space-y-8 items-center  justify-center text-center overflow-hidden'>
      <BackgroundCircles />
      <h2 className='text-sm uppercase tracking-[15px] pb-2 z-20'>{pageInfo?.role}</h2>
